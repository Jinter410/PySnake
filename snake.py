...
class Snake():
    def load_head_and_eyes(self):
        ...
        resized_head = pygame.transform.scale(
            head_full, (self.head_size, self.head_size))
        self.head = resized_head
        # Pareil pour les yeux
        eyes_full = pygame.image.load('./img/snake_eyes.png')
        resized_eyes = pygame.transform.scale(
            eyes_full, (self.eyes_size, self.eyes_size))
    ...
    def __init__(self, paramScreen):
        ...
        self.rect = pygame.Rect(self.x, self.y, self.head_size, self.head_size)
        # La hitbox du snake (circulaire maintenant)
        self.hitbox_radius = self.head_size / 2
    ...
    def mise_a_jour_corps(self):
        ...
        if len(self.bodyparts) > 0:
            for i in range(len(self.bodyparts)):
                ...
                if i == 0:
                    self.bodyparts[0].x_center += (
                        self.rect.centerx - self.bodyparts[0].x_center)*SPEED/50
                    self.bodyparts[0].y_center += (
                        self.rect.centery - self.bodyparts[0].y_center)*SPEED/50
                    self.bodyparts[0].update_coords_from_center()
                # Sinon on fait en fonction du morceau d'avant
                else:
                    self.bodyparts[i].x_center += (
                        self.bodyparts[i-1].x_center - self.bodyparts[i].x_center)*SPEED/50
                    self.bodyparts[i].y_center += (
                        self.bodyparts[i-1].y_center - self.bodyparts[i].y_center)*SPEED/50
    ...
    def spawn_body(self):
        ...
        if len(self.bodyparts) > 0:
            ...
            new_bodypart_x_center = last_bodypart.x_center + \
                NEW_BODYPART_X[self.direction] * self.head_size
            new_bodypart_y_center = last_bodypart.y_center + \
                NEW_BODYPART_Y[self.direction] * self.head_size
        # Sinon on fait la même chose en prenant la tête comme dernière partie
        else:
            new_bodypart_x_center = self.rect.centerx + \
                NEW_BODYPART_X[self.direction] * self.head_size
            new_bodypart_y_center = self.rect.centery + \
                NEW_BODYPART_Y[self.direction] * self.head_size
        # On crée la dernière partie du corps et on l'ajoute à la liste
        new_part = BodyPart(
            self.body_size, new_bodypart_x_center, new_bodypart_y_center)
...
class BodyPart():
    ...
    def load_body(self):
        ...
        resized_body = pygame.transform.scale(
            body_full, (self.size, self.size))
